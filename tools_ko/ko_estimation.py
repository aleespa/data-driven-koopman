...
class KoopmanEstimation:
    ...
    def bandwidth_search(self, h=1e-6, K=50, plot=False):
        ...
        alternate_Norm = -distance.squareform(distance.pdist(self.X, "sqeuclidean")) / (
            4
        )
        T = lambda alternate_epsilon: self.Manifold_volume_kernel(
            alternate_Norm, alternate_epsilon
        )
        a = np.zeros(K)
        ...
        self.d = (
            2
            * (log(T(h + self.epsilon)) - log(T(self.epsilon)))
            / (log(self.epsilon + h) - log(self.epsilon))
        )
