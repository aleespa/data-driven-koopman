...
def kernel_matrix(X, epsilon, epsilon_0=None):
    ...
    Norm = -distance.squareform(distance.pdist(X, "sqeuclidean")) / (
        4 * (Rho.T @ Rho) * epsilon
    )
    return np.exp(Norm)
...
def bandwidth_search(X, h=1e-6, K=50, verbose=False, plot=False):
    ...
    for i, e in enumerate(
        2 ** np.linspace(-20, 2, K)
    ):  # evaluate for valyes from 2^-30 to 2
    ...
    if plot:
        ...
        axs[1].plot(
            2 ** np.linspace(-20, 2, K),
            [T(2**l) for l in np.linspace(-20, 2)],
            zorder=1,
            color="#322671",
        )
...
def KNPGenerator(
    X, M, plot=False, return_extra=False, epsilon_0=None, epsilon=None, d=None
):
    """
