...
p_0 = sp.stats.norm.pdf(X,0,1) #Initial Distribution to apply Perron-Frobenius operator
...
plt.plot(np.linspace(min(X),max(X),100),K.p_est(np.linspace(min(X),max(X),100).T),
         color='r',lw=1)
plt.plot(np.linspace(min(X),max(X),100),sp.stats.norm.pdf(np.linspace(min(X),max(X),100),0,1),
         color='k',lw=1)
plt.xlabel('$x$')
plt.ylabel('$p(t,x)$')
plt.grid(alpha=0.3)
...
for j in range(Koopman.shape[0]):
    ax.plot(np.sort(X[:,0]), Koopman[j,np.argsort(X[:,0])],color=plt.cm.inferno(j/(Koopman.shape[0])))
# ax.set_xlim(-2,2)
ax.grid(alpha=0.3)
ax.set_xlabel('$x$')
ax.set_ylabel('$\mathcal{K}^{t}\exp(x)$')
fig.colorbar(plt.cm.ScalarMappable(norm=plt.Normalize(0,1), cmap=plt.cm.inferno),ax= ax,label='Time, t')
plt.show()